...
状态：进行中。Analytics 与 Spring AI Copilot 已完成；LangGraph Agent、Agent tools 和 MCP 仍待开发。

交付内容：

- 使用可配置 provider 接入 Spring AI。
- 实现 AI availability、issue breakdown、issue summary 和 project summary。
- 使用 `AiSuggestion` 保存经过校验的 AI 草稿，用户编辑和确认后才写入正式业务数据。
- 新增 Python + FastAPI + LangGraph Project Planning Agent。
- Agent 支持 FlowAI tools、checkpoint、澄清问题、人工审批、恢复和取消。
- Spring Boot 保持唯一业务数据入口，Python Agent 不直接修改 FlowAI 业务表。
- 后续把第一批只读 FlowAI tools 暴露为 MCP。
- Analytics overview 和前端统计页面已经完成。
- Issue Breakdown 已完成后端 API、结构化输出校验、一次修复、tenant/project 权限检查和 DRAFT suggestion 持久化。
- Suggestion Get/Dismiss/Expire 与事务化幂等 Apply 已完成，Apply 复用普通 Issue 创建校验并支持整批回滚。
- Issue Detail 已完成 Breakdown/Summary 抽屉；Analytics 已完成 Project Summary 生成、恢复、复制和刷新。
- Issue Summary 与 Project Summary 已完成受限上下文、服务端来源统计、截断提示、一次修复和草稿持久化。
- 三个生成入口共用用户/workspace 限流，并已接入通用 token、耗时、结果和 suggestion/apply metrics。
- 已增加默认跳过、仅显式环境变量启用的真实 OpenAI Provider smoke integration test。
...
- Agent 在澄清和审批节点可以暂停、持久化并恢复。
- Agent 只能读取当前用户有权访问的数据，正式写入必须再次检查权限并获得人工批准。
- AI 和 Agent 在 CI 中使用确定性 fake model，不依赖真实 provider。

详细功能、架构、接口、数据模型、安全、测试和分阶段计划见 [Phase 4 AI Copilot 与 Agent 工作流设计](./phase-4-ai-agent.zh-CN.md)。
